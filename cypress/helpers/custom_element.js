export const customElement = (selector) => {
  const element = {
    ...
    haveText(text, ignoreWhitespace = true) {
      text = String(text);
      const asseredText = ignoreWhitespace ? text.trim() : text; // ? Pokud nebude fungovat trim, tak použít text.replace(/\s+/g, ' ')
      getCypressElement(selector).should("have.text", text);
      return this;
    },
    containsText(text) {
